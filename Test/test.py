...
def test_register_valid():
    response = requests.post(
        url+'register', json={"firstName": 'first', "lastName": 'second', "password": 'password1', "email": 'abc@agmail.com'})
...
def test_login_valid():
    response1 = requests.post(
        url+'login', json={"password": 'password1', "email": 'abc@agmail.com'})
    assert response1.status_code == 200
    assert response1.json()['email'] == 'abc@agmail.com'
    assert response1.json()['firstName'] == 'first'
    assert response1.json()['lastName'] == 'second'
...
#     response1 = requests.post(url+'login', json= {"password" : 'password1', "email": 'abc@agmail.com'})
#     response1 = requests.post(url+'logout', json= {"token": response1.json()['token']})
